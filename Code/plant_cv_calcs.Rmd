...
quantileRange <- function(x, low = .1, high = .9) {
  (quantile(x, low) - quantile(x, high)) / mean(density)
}

# coefficient of variation
coeffVar <- function(x) sd(biomass)/mean(biomass)

# metrics based on analysis of slopes
slopeMetrics <- function(x, radians = TRUE) {
...
temp_cv_quad <- ab %>%
  ...
  summarize(cv = coeffVar(biomass), # coefficient of variation
            ...
            acv = acv(biomass),
            q1090 = (quantile(biomass, 0.9) - quantile(density, 0.1)) / mean(density),
...
temp_cv_quad <- ab %>%
  ...
  # gather metrics of bounciness
  pivot_longer(cols=c(cv, q1090), names_to="bounce_type", values_to = "bounce")
```
