...
pics(x=x, trait="ldmc", phy=read.tree("Data/sev_traits_tree101.tre"), taxa="taxon.new")
```
...
brl_transform <- function(x, # a tibble or data frame with trait data
  ...
  # All equal
  pic.all1 <- list(pics(x=x, trait=trait, phy=compute.brlen(phy=pruned, runif, min=0, max=1), 
  ...
  pic.pagel <- lapply(lambdas, function(l) {
    ...
    phy.rscl <- rescale(x=phy, model="lambda", l)
    pics(x=x, trait=trait, phy=phy.rscl, taxa=taxa, scale=scale) %>%
      as_tibble() %>%
  ...
  pics.out <- c(pic.raw=pic.raw, pic.all1=pic.all1, pic.pagel)
  
  return(pics.out)
  
}

pics.ldmc <- brl_transform(x=x, trait="ldmc", phy=read.tree("Data/sev_traits_tree101.tre"), taxa = "taxon.new")

```

```{r Visualize Transformations}

pics.ldmc.df <- do.call(rbind, pics.ldmc) %>%
  mutate(pic_abs = abs(contrasts), 
         pic_sd = sqrt(variance)) %>%
  select(pic = contrasts, pic_abs, var=variance, pic_sd, trans)

pdf(file = paste("Figures/", "pic_transformations_ldmc.pdf"))
