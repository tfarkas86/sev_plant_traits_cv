...
zranks <- temp_cv %>%
  filter(site == "F", 
         ...
         season == "F", 
         bounce_type == "cv") %>%
  mutate(zrank = 100-rank(n_zero_mean)) %>%
  select(kartez, zrank) 

# join in rank to all traits data
all_traits2 <- all_traits %>%
  left_join(zranks) %>%
  select(kartez, zrank, family:last_col()) %>%
  drop_na(zrank)

ranks <- unique(zranks$zrank)[order(unique(zranks$zrank))]
...
rare_list <- lapply(ranks[1:30], function(rank) {

all_pics_list <- setNames(lapply(names(trans), function(t) {
  
 all_trans <- brl_transform(x=all_traits2 %>% filter(zrank >= rank), 
                            trait=t, phy=tree,
               taxa = "taxon.new")
 all_trans[[trans_options[trans[t]]]]
