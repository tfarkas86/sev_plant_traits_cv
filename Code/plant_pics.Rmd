...
save(rare_list,file= "~/Dropbox/Projects/Sev/Plants/sev_traits_cv_manuscript/Data/rare_list.RData")
...
map(c("linear", "quadratic", "cubic"), .f = function(fit) {
  ...
  map(as.character(inranks[1:2]), function(set) {
    ...
    map(c("cv", "q1090"), function(metric) {
      ...
      map(f.list[1:2], function(trait) {
        
        paste(trait, metric, fit)
        form_str <- switch(as.character(fit), 
                           ...
                           "cubic" = paste0("F_F_BM_", metric, 
                                        " ~ pic + I(pic^2) + I(pic^3)"))
        lm(formula(form_str), 
            data=pd_long %>% filter(trait==trait))
        
      }) %>% set_names(f.list[1:2])
    }) %>% set_names(c("cv", "q1090"))
  }) %>% set_names(as.character(inranks[1:2]))
...
cv_mods <- c(lapply(setNames(f.list, f.list), function(x) {
  
 lm(F_F_BM_cv ~ pic, 
            data=pd_long %>% filter(trait==x))
  
}), recursive=FALSE)

cv_pees <- c(map(cv_mods, ~ summary(.x)$coefficients[2, 4]), 
              recursive=TRUE)

cv_sigs <- ifelse(cv_pees > 0.10, "", 
               ifelse(cv_pees > 0.05, "#",
                      ifelse(cv_pees > 0.01, "*", 
                             ifelse(cv_pees > 0.001, "**", "***"))))

cv_r2 <- c(map(cv_mods, ~ summary(.x)$adj.r.squared), recursive = TRUE)

# q1090
q1090_mods <- c(lapply(setNames(f.list, f.list), function(x) {
...
q1090_pees <- c(map(q1090_mods, ~ summary(.x)$coefficients[2, 4]), 
              recursive=TRUE)

q1090_sigs <- ifelse(q1090_pees > 0.10, "", 
               ifelse(q1090_pees > 0.05, "#",
                      ifelse(q1090_pees > 0.01, "*", 
                             ifelse(q1090_pees > 0.001, "**", "***"))))

q1090_r2 <- c(map(q1090_mods, ~ summary(.x)$adj.r.squared), recursive = TRUE)
